...
func (controller *UserController) GetHistory(c echo.Context) error {
	user := c.Get("user").(*jwt.Token)
	claims, _ := user.Claims.(*helper.CustomClaims)
	userID := claims.UserID
	...
	return c.JSON(http.StatusOK, helper.ResponseToClient(http.StatusOK, "Get history success", result))
}
